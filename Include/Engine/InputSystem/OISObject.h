...
namespace OIS
{
	...
	public:
		...
		virtual bool IsBuffered() const { return m_isBuffered; }

		/** @remarks Returns this input object's creator */
		InputSystem* getCreator() const { return mCreator; }
	...
	protected:
		Object(const std::string &vendor, Type iType, bool buffered, int devID, InputSystem* creator) 
			: mVendor(vendor), mType(iType),m_isBuffered(buffered), mDevID(devID),mCreator(creator) 
		{}

	protected:
		std::string   mVendor;		//! Vendor name if applicable/known		
		Type		  mType;		//! Type of controller object	
		bool		  m_isBuffered;	//! Buffered flag
		int			  mDevID;		//! Not fully implemented yet
		InputSystem* mCreator;		//! The creator who created this object
	};
}
#endif
