...
namespace OIS
{
	...
	class Win32InputManager : public InputSystem, public FactoryCreator
	{
	public:
		Win32InputManager();
		...
		Object* createObject(InputSystem* creator, Type iType, bool bufferMode, const std::string & vendor = "");
	...
	private:
		JoyStickInfoList	unusedJoyStickList;		//! Keep a list of all joysticks enumerated, but not in use
		HWND				hWnd;					//! The window handle we are using
		IDirectInput8*		mDirectInput;			//! Direct Input Interface
		DWORD				kbSettings;				//! Used for keyboard device settings
		DWORD				m_mouseSettings;		//! Used for mouse device settings
		DWORD				joySettings;			//! Used for joystick device settings
		char				joySticks;				//! Number of total joysticks (inuse or not)
		bool				keyboardUsed;			//! Used to know if we used up keyboard
		bool				mouseUsed;				//! Used to know if we used up mouse
	};
}
#endif
