...
namespace MyGUI
{
	...
	class MYGUI_EXPORT InputManager : public Ares::Singleton<InputManager>, public IUnlinkWidget, public MemberObsolete<InputManager>
	{
	public:
		InputManager();
		...
		*/
		...
		/** Inject MousePress event
			@return true if event has been processed by GUI
		*/
		bool injectMousePress(int _absx, int _absy, OIS::MouseButtonID _id);
		/** Inject MouseRelease event
			@return true if event has been processed by GUI
		*/
		bool injectMouseRelease(int _absx, int _absy, OIS::MouseButtonID _id);

		/** Inject KeyPress event
			@return true if event has been processed by GUI
		*/
		bool injectKeyPress( OIS::KeyCode _key, Char _text = 0);
		/** Inject KeyRelease event
			@return true if event has been processed by GUI
		*/
		bool injectKeyRelease( OIS::KeyCode _key);
		...
		*/
		const IntPoint& getLastPressedPosition(OIS::MouseButtonID _id) const;

		/** Get current mouse position on screen */
		const IntPoint& getMousePosition() const { return m_mousePosition; }
	...
	private:
		...
		void firstEncoding( OIS::KeyCode _key, bool bIsKeyPressed);

		// запоминает клавишу для поддержки повторения
		void storeKey( OIS::KeyCode _key, Char _text);
	...
	private:
		...
		bool		mIsShiftPressed;				// нажат ли шифт
		bool		mIsControlPressed;				// нажат ли контрол
		IntPoint	m_mousePosition;					// 鼠标位置
		IntPoint	mLastPressed[OIS::MB_Max];		// last mouse press position
		bool		mMouseCapture[OIS::MB_Max];		// is mouse button captured by active widget

		// клавиша для повтора
		OIS::KeyCode	mHoldKey;
		Char			mHoldChar;
		bool			mFirstPressKey;
		float			mTimerKey;
		int				mOldAbsZ;
