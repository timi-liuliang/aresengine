...
namespace MyGUI
{
	...
	//--------------------------------------
	// 界面系统主入口类 2013-4-23 帝林<修改>
	//--------------------------------------
	class MYGUI_EXPORT Gui : public Ares::Singleton<Gui>, public IUnlinkWidget, public MemberObsolete<Gui>
	{
	...
	private:
		...
		// синглтоны гуя
		InputManager*		m_inputMgr;			// 输入管理器
		SubWidgetManager*	mSubWidgetManager;
		LayerManager*		m_layerMgr;			// 层级管理器
		SkinManager*		m_skinMgr;			// 皮肤管理器
	...
	#define AUiLayerMgr  LayerManager::GetSingleton()		// 层管理器
	#define AUiLayoutMgr LayoutManager::GetSingleton()		// 布局管理器
	#define AUiSkinMgr   SkinManager::GetSingleton()		// 皮肤管理器
	#define AUiInputMgr  InputManager::GetSingleton()		// 输入管理器
