...
namespace MyGUI
{
	...
	struct MYGUI_EXPORT IBNotifyItemData
	{
		...
		IBNotifyItemData(size_t _index, NotifyItem _notify, int _x, int _y, OIS::MouseButtonID _id) :
			index(_index),
			notify(_notify),
			x(_x),
			y(_y),
		...
		IBNotifyItemData(size_t _index, NotifyItem _notify, OIS::KeyCode _code, Char _key) :
			index(_index),
			notify(_notify),
			x(0),
			y(0),
		...
		IBNotifyItemData(size_t _index, NotifyItem _notify, OIS::KeyCode _code) :
			index(_index),
			notify(_notify),
			x(0),
			y(0),
