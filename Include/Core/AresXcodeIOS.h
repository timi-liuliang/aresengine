...
typedef boost::uint32_t  DWORD;		// 32-bit unsigned
...
#define GET_VARARGS_RESULT( result, len, fmt) { va_list args; va_start(args, fmt); vsnprintf( result, len, fmt, args); va_end(args); }
