...
namespace Ares
{
	...
	void Camera::SetViewParams( Vector3* vPos, Vector3* vLookAt, Vector3* vUp)
	{
	...
	// 设置投影矩阵参数
	void Camera::SetProjParams( float fovH, int screenWidth, int screenHeight, float fNearPlane, float fFarPlane, RenderDeviceType type)
	{
		m_fovH			= fovH;
	...
	// 设置摄像机位置
	void Camera::SetPos(Vector3* vPos)
	{
		m_vPosition = *vPos;
		UpdateViewMatrix();
	}

	void Camera::SetLookAt(Vector3* v_pLookAt)
	{
		m_vLookAt = *v_pLookAt;
	}
	...
	void Camera::RefreshProjOrthoMatrix()
	{
		// 正交矩阵
	...
	// 更新
	BOOL Camera::FrameMove(float fTime)
	{ 			
		//UpdatePosOrLookAt();
	...
	// 正交矩阵
	const Matrix44& Camera::GetOrtho()
	{
		return m_matOrtho;
	}

	// 观察正交矩阵
	const Matrix44& Camera::GetViewOrtho()
	{
