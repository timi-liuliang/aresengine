#include "Engine/InputSystem/OISConfig.h"
...
void LIRCFactoryCreator::enableConnectionThread(bool enable)
{
	...
	{
		mThreadRunning = true;
		mWrapped->mThreadHandler = new_ boost::thread(boost::bind(&LIRCFactoryCreator::threadUpdate, this));
	}
...
//---------------------------------------------------------------------------------//
Object* LIRCFactoryCreator::createObject(InputSystem* creator, Type iType, bool bufferMode, const std::string & vendor)
{
	if( mUnusedRemotes.size() > 0 )
	{
		...
		if( remote != mUnusedRemotes.end() )
		{
			...
			//Create device
			LIRCControl *obj = new_ LIRCControl(creator, 0, bufferMode, this, mJoyStickInformation[*remote]);
