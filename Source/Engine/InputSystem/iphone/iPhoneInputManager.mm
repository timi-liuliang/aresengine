...
#include "iPhoneInputManager.h"
#include "iPhoneHelpers.h"
#include "iPhoneAccelerometer.h"
#include "iPhoneMultiTouch.h"
...
//--------------------------------------------------------------------------------//
iPhoneInputManager::iPhoneInputManager() : InputSystem("iPhone Input Manager")
{
    mHideMouse = true;
...
DeviceList iPhoneInputManager::freeDeviceList()
{
	...
	if( bAccelerometerUsed == false )
		ret.insert(std::make_pair(OISJoyStick, m_inputSystemName));

	if( bMultiTouchUsed == false )
		ret.insert(std::make_pair(OISMultiTouch, m_inputSystemName));
...
{
	if( ( iType == OISMultiTouch || iType == OISJoyStick ) && vendor == m_inputSystemName )
		return true;
...
//--------------------------------------------------------------------------------//
Object* iPhoneInputManager::createObject(InputSystem* creator, Type iType, bool bufferMode,
