...
#include <Engine/InputSystem/OISEvents.h>
#include <Engine/AresRoot.h>

using namespace OIS;

// 构造函数
Win32Mouse::Win32Mouse( InputSystem* creator, IDirectInput8* pDI, bool buffered, DWORD coopSettings )
	: Mouse(creator->inputSystemName(), buffered, 0, creator)
{
	mMouse		 = 0;
	mDirectInput = pDI;
	m_coopSetting= coopSettings;
	mHwnd		 = 0;
...
void Win32Mouse::_initialize()
{
	...
	if( FAILED( mMouse->SetCooperativeLevel(mHwnd, m_coopSetting)))
		ALogSystem.Warning( "Win32Mouse::Win32Mouse >> Failed to set coop level");
		...
		if( !m_listeners.empty() && m_isBuffered )
		{
			for( size_t i=0; i<m_listeners.size(); i++)
			{
				if( m_listeners[i]->IsValid())
					m_listeners[i]->mouseMoved( MouseEvent( this, mState));
			}
		}
	}
}
...
bool Win32Mouse::_doMouseClick( int mouseButton, DIDEVICEOBJECTDATA& di )
{
	if( di.dwData & 0x80 )
	{
		...
		if( !m_listeners.empty() && m_isBuffered )
		{
			...
			{
				if( m_listeners[i]->IsValid())
					m_listeners[i]->mousePressed( MouseEvent( this, mState ), (MouseButtonID)mouseButton );
			}
		}
	}
	else
	{
		...
		if( !m_listeners.empty() && m_isBuffered )
		{
			...
			{
				if( m_listeners[i]->IsValid())
					m_listeners[i]->mouseReleased( MouseEvent( this, mState ), (MouseButtonID)mouseButton );
			}
		}
	}
