...
#include "Engine/InputSystem/win32/Win32ForceFeedback.h"
...
//--------------------------------------------------------------------------------------------------//
Win32JoyStick::Win32JoyStick( InputSystem* creator, IDirectInput8* pDI, bool buffered, DWORD coopSettings, const JoyStickInfo &info ) :
...
BOOL CALLBACK Win32JoyStick::DIEnumEffectsCallback(LPCDIEFFECTINFO pdei, LPVOID pvRef)
{
	...
	if( _this->mFfDevice == 0 )
		_this->mFfDevice = new_ Win32ForceFeedback(_this->mJoyStick, &_this->mDIJoyCaps);
...
void Win32JoyStick::capture()
{
	...
	if( m_isBuffered && mListener && entries > 0 )
	{
