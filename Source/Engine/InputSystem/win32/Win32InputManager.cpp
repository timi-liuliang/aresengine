...
#include "Engine/InputSystem/win32/Win32KeyBoard.h"
#include "Engine/InputSystem/win32/Win32Mouse.h"
#include "Engine/InputSystem/win32/Win32JoyStick.h"
...
//--------------------------------------------------------------------------------//
Win32InputManager::Win32InputManager() : InputSystem("Win32InputManager")
{
	hWnd = 0;
	mDirectInput	 = 0;
...
void Win32InputManager::_parseConfigSettings( ParamList &paramList )
{
	...
	for( ; i != e; ++i ) 
	{
		if( i->first == "w32_keyboard" )
				...
				m_mouseSettings |= temp[i->second];
		else if( i->first == "w32_joystick" )
				joySettings |= temp[i->second];
	}
	if( kbSettings == 0 ) 
		kbSettings = DISCL_FOREGROUND | DISCL_NONEXCLUSIVE | DISCL_NOWINKEY;

	if( !m_mouseSettings) 
		m_mouseSettings = DISCL_FOREGROUND | DISCL_NONEXCLUSIVE;

	if( joySettings == 0 ) 
		joySettings = DISCL_FOREGROUND | DISCL_EXCLUSIVE;
}
...
DeviceList Win32InputManager::freeDeviceList()
{
	...
	if( keyboardUsed == false )
		ret.insert(std::make_pair(OISKeyboard, m_inputSystemName));

	if( mouseUsed == false )
		ret.insert(std::make_pair(OISMouse, m_inputSystemName));
...
{
	if( (iType == OISKeyboard || iType == OISMouse) && vendor == m_inputSystemName )
	{
		return true;
	}
...
//----------------------------------------------------------------------------//
Object* Win32InputManager::createObject(InputSystem* creator, Type iType, bool bufferMode, const std::string & vendor)
{
	Object *obj = 0;
	...
	{
		if( keyboardUsed == false )
			obj = new_ Win32Keyboard(this, mDirectInput, bufferMode, kbSettings);
		break;
	}
	case OISMouse:
	{
		if( mouseUsed == false )
			obj = new_ Win32Mouse(this, mDirectInput, bufferMode, m_mouseSettings);
		break;
	}
	case OISJoyStick:
	{
		for(JoyStickInfoList::iterator i = unusedJoyStickList.begin(); i != unusedJoyStickList.end(); ++i)
		{
			...
			{
				obj = new_ Win32JoyStick(this, mDirectInput, bufferMode, joySettings, *i);
