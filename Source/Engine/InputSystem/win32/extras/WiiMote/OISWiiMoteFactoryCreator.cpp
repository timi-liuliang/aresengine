	...
	mtThreadRunning(0)
{
	...
	mtWiiMoteListMutex = new_ boost::mutex();
}
...
//---------------------------------------------------------------------------------//
Object* WiiMoteFactoryCreator::createObject(InputSystem* creator, Type iType, bool bufferMode, const std::string & vendor)
{
	if( mFreeWiis.size() > 0 && (vendor == "" || vendor == mVendorName ) )
	{
		int id = mFreeWiis.front();
		mFreeWiis.pop_front();
		WiiMote *wii = new_ WiiMote(creator, id, bufferMode, this);

		if( mtThreadRunning == false )
		{	//Create common thread manager (this is the first wiimote created)
			mtThreadRunning = true;
			mtThreadHandler = new_ boost::thread(boost::bind(&WiiMoteFactoryCreator::_updateWiiMotesThread, this));
		}
