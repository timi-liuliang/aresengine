...
namespace MyGUI
{
	...
	void DDContainer::onMouseButtonPressed(int _left, int _top, OIS::MouseButtonID _id)
	{
		// смещение внутри виджета, куда кликнули мышкой
		mClickInWidget = InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left) - getAbsolutePosition();
	...
	void DDContainer::onMouseButtonReleased(int _left, int _top, OIS::MouseButtonID _id)
	{
	...
	void DDContainer::onMouseDrag(int _left, int _top, OIS::MouseButtonID _id)
	{
		mouseDrag(_id);
	...
	void DDContainer::mouseButtonPressed(OIS::MouseButtonID _id)
	{
		if ( OIS::MB_Left == _id)
		{
	...
	void DDContainer::mouseButtonReleased( OIS::MouseButtonID _id)
	{
		if ( OIS::MB_Left == _id)
		{
			endDrop(false);
	...
	void DDContainer::mouseDrag( OIS::MouseButtonID _id)
	{
		if ( OIS::MB_Left != _id)
			return;
		...
		if (!mStartDrop && mDropSenderIndex != ITEM_NONE)
		{
			...
			else
			{
				...
				InputManager::GetSingleton().resetMouseCaptureWidget();
			}
		}
		...
		Widget* item = LayerManager::GetSingleton().getWidgetFromPoint(point.left, point.top);
