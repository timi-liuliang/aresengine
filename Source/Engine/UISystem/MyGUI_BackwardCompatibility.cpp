...
#include "Engine/UISystem/MyGUI_DeprecatedWidgets.h"
...
namespace MyGUI
{
	...
	{
		return ResourceManager::GetSingleton().load(_file);
	}
	...
	{
		return InputManager::GetSingleton().injectMouseMove(_absx, _absy, _absz);
	}
	bool MemberObsolete<Gui>::injectMousePress( int _absx, int _absy, OIS::MouseButtonID _id )
	{
		return InputManager::GetSingleton().injectMousePress(_absx, _absy, _id);
	}
	bool MemberObsolete<Gui>::injectMouseRelease( int _absx, int _absy, OIS::MouseButtonID _id )
	{
		return InputManager::GetSingleton().injectMouseRelease(_absx, _absy, _id);
	}
	bool MemberObsolete<Gui>::injectKeyPress( OIS::KeyCode _key, Char _text)
	{
		return InputManager::GetSingleton().injectKeyPress(_key, _text);
	}
	bool MemberObsolete<Gui>::injectKeyRelease( OIS::KeyCode _key)
	{
		return InputManager::GetSingleton().injectKeyRelease(_key);
	}

	void MemberObsolete<Gui>::hidePointer()
	{
		PointerManager::GetSingleton().setVisible(false);
	}
	void MemberObsolete<Gui>::showPointer()
	{
		PointerManager::GetSingleton().setVisible(true);
	}
	bool MemberObsolete<Gui>::isShowPointer()
	{
		return PointerManager::GetSingleton().isVisible();
	}
	...
	{
		return RenderManager::GetSingleton().getViewSize();
	}
	int MemberObsolete<Gui>::getViewWidth()
	{
		return RenderManager::GetSingleton().getViewSize().width;
	}
	int MemberObsolete<Gui>::getViewHeight()
	{
		return RenderManager::GetSingleton().getViewSize().height;
	}

	void MemberObsolete<Gui>::setVisiblePointer(bool _value)
	{
		PointerManager::GetSingleton().setVisible(_value);
	}
	...
	{
		Gui::GetSingleton().frameEvent(_time);
	}

	void MemberObsolete<Gui>::_resizeWindow(const IntSize& _size)
	{
		LayerManager::GetSingleton().resizeView(_size);
	}

	const IntPoint& MemberObsolete<InputManager>::getLastLeftPressed() const
	{
		return InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left);
	}

	const IntPoint& MemberObsolete<InputManager>::getLastRightPressed() const
	{
		return InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Right);
	}
	...
	bool BackwardCompatibility::checkProperty(Widget* _owner, std::string& _key, std::string& _value)
	{
		...
		{
			MYGUI_LOG(Warning, (*item).first << " is deprecated, use " << (*item).second << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			_key = (*item).second;
		}
		else if (_key == "Progress_StartPoint")
		{
			MYGUI_LOG(Warning, "Progress_StartPoint is deprecated, use Progress_FlowDirection" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			_key = "Progress_FlowDirection";
			_value = convertAlignToDirection(_value);
		}
		else if (_key == "ComboBox_AddItem")
		{
			MYGUI_LOG(Warning, "ComboBox_AddItem is deprecated" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
		...
		{
			MYGUI_LOG(Warning, "List_AddItem is deprecated" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
		...
		{
			MYGUI_LOG(Warning, "Tab_AddSheet is deprecated" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
		...
		{
			MYGUI_LOG(Warning, "Tab_AddItem is deprecated" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			TabControl* widget = _owner->castType<TabControl>(false);
			if (widget != nullptr)
				widget->addItem(_value);
			return false;
		...
		{
			MYGUI_LOG(Warning, "Window_MinMax is deprecated, use Window_MinSize or Window_MaxSize" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
		...
		{
			MYGUI_LOG(Warning, "ImageRect is deprecated, use ImageCoord" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			_key = "ImageCoord";
			_value = convertRectToCoord(_value);
		}
		else if (_key == "StartPoint")
		{
			MYGUI_LOG(Warning, "StartPoint is deprecated, use FlowDirection" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			_key = "FlowDirection";
			_value = convertAlignToDirection(_value);
		}
	...
	std::string BackwardCompatibility::getFactoryRename(const std::string& _categoryName, const std::string& _factoryName)
	{
		...
		if (_categoryName == "Widget")
		{
			...
			{
				MYGUI_LOG(Warning, "HScroll factory is deprecated, use " << ScrollBar::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "VScroll")
			{
				MYGUI_LOG(Warning, "VScroll factory is deprecated, use " << ScrollBar::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "RenderBox")
			{
				MYGUI_LOG(Warning, "RenderBox factory is deprecated, use " << Canvas::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "Sheet")
			{
				MYGUI_LOG(Warning, "Sheet factory is deprecated, use " << TabItem::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "StaticImage")
			{
				MYGUI_LOG(Warning, "StaticImage factory is deprecated, use " << ImageBox::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "StaticText")
			{
				MYGUI_LOG(Warning, "StaticText factory is deprecated, use " << TextBox::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "Progress")
			{
				MYGUI_LOG(Warning, "Progress factory is deprecated, use " << ProgressBar::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "List")
			{
				MYGUI_LOG(Warning, "List factory is deprecated, use " << ListBox::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "Edit")
			{
				MYGUI_LOG(Warning, "Edit factory is deprecated, use " << EditBox::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "Tab")
			{
				MYGUI_LOG(Warning, "Tab factory is deprecated, use " << TabControl::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "MultiList")
			{
				MYGUI_LOG(Warning, "MultiList factory is deprecated, use " << MultiListBox::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
			else if (_factoryName == "MenuCtrl")
			{
				MYGUI_LOG(Warning, "MenuCtrl factory is deprecated, use " << MenuControl::getClassTypeName() << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			}
		}
#endif // MYGUI_DONT_USE_OBSOLETE
		return _factoryName;
	...
	std::string BackwardCompatibility::getSkinRename(const std::string& _skinName)
	{
		...
		{
			MYGUI_LOG(Warning, (*item).first << " skin is deprecated, use " << (*item).second << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			return (*item).second;
		}
#endif // MYGUI_DONT_USE_OBSOLETE
		return _skinName;
