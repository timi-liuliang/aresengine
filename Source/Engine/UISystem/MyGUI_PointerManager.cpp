...
#include "Engine/UISystem/MyGUI_ResourceManualPointer.h"
#include "Engine/UISystem/MyGUI_ResourceImageSetPointer.h"

namespace Ares
{
	using namespace MyGUI;

	template <> PointerManager* Ares::Singleton<PointerManager>::m_singleton = nullptr;
...
namespace MyGUI
{
	...
	void PointerManager::initialise()
	{
		...
		Gui::GetSingleton().eventFrameStart += newDelegate(this, &PointerManager::notifyFrameStart);
		InputManager::GetSingleton().eventChangeMouseFocus += newDelegate(this, &PointerManager::notifyChangeMouseFocus);
		WidgetManager::GetSingleton().registerUnlinker(this);

		ResourceManager::GetSingleton().registerLoadXmlDelegate(XML_TYPE) = newDelegate(this, &PointerManager::_load);

		FactoryManager::GetSingleton().registerFactory<ResourceManualPointer>(XML_TYPE_RESOURCE);
		FactoryManager::GetSingleton().registerFactory<ResourceImageSetPointer>(XML_TYPE_RESOURCE);
	...
	void PointerManager::shutdown()
	{
		...
		FactoryManager::GetSingleton().unregisterFactory<ResourceManualPointer>(XML_TYPE_RESOURCE);
		FactoryManager::GetSingleton().unregisterFactory<ResourceImageSetPointer>(XML_TYPE_RESOURCE);
		...
		while (node.next())
		{
			if (node->getName() == XML_TYPE)
			{
				...
				while (info.next("Info"))
				{
					...
					ResourceManager::GetSingleton().loadFromXmlNode(root, _file, _version);
				}
	...
	{
		mPoint = InputManager::GetSingleton().getMousePosition();
	...
	{
		Widget* widget = WidgetManager::GetSingleton().createWidget(_style, _type, _skin, _coord, /*_align, */nullptr, nullptr, /*this, */_name);
	...
	void PointerManager::setLayerName(const std::string& _value)
	{
		...
		mLayerName = _value;
		if (LayerManager::GetSingleton().isExist(_value))
			LayerManager::GetSingleton().attachToLayerNode(mLayerName, mMousePointer);
	}
	...
	IPointer* PointerManager::getByName(const std::string& _name) const
	{
		...
		if (!_name.empty() && _name != RESOURCE_DEFAULT_NAME)
			result = ResourceManager::GetSingleton().getByName(_name, false);
