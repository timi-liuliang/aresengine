...
namespace MyGUI
{
	...
	void MultiListBox::frameAdvise(bool _advise)
	{
		if (_advise)
		{
			if (!mFrameAdvise)
			{
				MyGUI::Gui::GetSingleton().eventFrameStart += MyGUI::newDelegate( this, &MultiListBox::frameEntered );
				mFrameAdvise = true;
			}
		}
		else
	...
	void MultiListBox::removeColumnAt(size_t _column)
	{
		...
		WidgetManager::GetSingleton().destroyWidget(info.item);
	}
		...
		for (VectorColumnInfo::iterator item = mVectorColumnInfo.begin(); item != mVectorColumnInfo.end(); ++item)
		{
			if ((*item).item == _item)
			{
				if ((*item).button != nullptr)
					WidgetManager::GetSingleton().destroyWidget((*item).button);
