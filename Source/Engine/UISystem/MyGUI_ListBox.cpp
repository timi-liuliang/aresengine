...
namespace MyGUI
{
	...
	void ListBox::onKeyButtonPressed( OIS::KeyCode _key, Char _char)
	{
		if (getItemCount() == 0)
		{
	...
	void ListBox::notifyMousePressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		if ( OIS::MB_Left != _id)
			return;
	...
	void ListBox::_redrawItemRange(size_t _start)
	{
		...
		else
		{
			...
			for (; pos < mWidgetLines.size(); pos++)
			{
				...
				//if (focus == mWidgetLines[pos]) InputManager::GetSingleton()._unlinkWidget(focus);
			}
		}
