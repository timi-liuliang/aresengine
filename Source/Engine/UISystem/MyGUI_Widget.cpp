...
#include "Engine/UISystem/MyGUI_WidgetDefines.h"
...
#include "Engine/UISystem/MyGUI_ISubWidget.h"
#include "Engine/UISystem/MyGUI_ISubWidgetText.h"
#include "Engine/UISystem/MyGUI_TextBox.h"
#include "Engine/UISystem/MyGUI_FactoryManager.h"
#include "Engine/UISystem/MyGUI_CoordConverter.h"
...
namespace MyGUI
{
	...
	void Widget::_initialise(WidgetStyle _style, const IntCoord& _coord, const std::string& _skinName, Widget* _parent, ICroppedRectangle* _croppedParent, const std::string& _name)
	{
		...
		if (LayoutManager::GetSingleton().isExist(_skinName))
			templateInfo = LayoutManager::GetSingleton().getByName(_skinName);
		else
			skinInfo = SkinManager::GetSingleton().getByName(_skinName);
	...
	const WidgetInfo* Widget::initialiseWidgetSkinBase(ResourceSkin* _skinInfo, ResourceLayout* _templateInfo)
	{
		...
		if (_skinInfo == nullptr)
		{
			...
			_skinInfo = SkinManager::GetSingleton().getByName(skinName);
		}
	...
	Widget* Widget::baseCreateWidget(WidgetStyle _style, const std::string& _type, const std::string& _skin, const IntCoord& _coord, Align _align, const std::string& _layer, const std::string& _name, bool _template)
	{
		...
		if (_template)
		{
			widget = WidgetManager::GetSingleton().createWidget(_style, _type, _skin, _coord, this, _style == WidgetStyle::Popup ? nullptr : this, _name);
	...
	void Widget::_destroyChildWidget(Widget* _widget)
	{
		...
		if (iter != mWidgetChild.end())
		{
			...
			// отписываем от всех
			WidgetManager::GetSingleton().unlinkFromUnlinkers(_widget);
	...
	{
		setPosition(CoordConverter::convertFromRelative(_point, mCroppedParent == nullptr ? RenderManager::GetSingleton().getViewSize() : mCroppedParent->getSize()));
	}

	void Widget::setRealSize(const FloatSize& _size)
	{
		setSize(CoordConverter::convertFromRelative(_size, mCroppedParent == nullptr ? RenderManager::GetSingleton().getViewSize() : mCroppedParent->getSize()));
	}

	void Widget::setRealCoord(const FloatCoord& _coord)
	{
		setCoord(CoordConverter::convertFromRelative(_coord, mCroppedParent == nullptr ? RenderManager::GetSingleton().getViewSize() : mCroppedParent->getSize()));
	}
	...
	void Widget::detachFromWidget(const std::string& _layer)
	{
		...
		if (parent)
		{
			...
			Gui::GetSingleton()._linkChildWidget(this);
		...
		if (!_layer.empty())
		{
			LayerManager::GetSingleton().attachToLayerNode(_layer, this);
		}
		else if (!oldlayer.empty())
		{
			LayerManager::GetSingleton().attachToLayerNode(oldlayer, this);
		}
	...
	void Widget::attachToWidget(Widget* _parent, WidgetStyle _style, const std::string& _layer)
	{
		...
		if (_style == WidgetStyle::Popup)
		{
			...
			if (mParent == nullptr)
				Gui::GetSingleton()._unlinkChildWidget(this);
			else
				mParent->_unlinkChildWidget(this);
		...
		{
			LayerManager::GetSingleton().detachFromLayer(this);
	...
	void Widget::_updateVisible()
	{
		...
		if (!value && InputManager::GetSingleton().getMouseFocusWidget() == this)
			InputManager::GetSingleton()._resetMouseFocusWidget();
	}
	...
	void Widget::_updateEnabled()
	{
		...
		if (!value)
			InputManager::GetSingleton().unlinkWidget(this);
	}
	...
	{
		if (getNeedToolTip())
			ToolTipManager::GetSingleton()._unlinkWidget(this);
	}
	...
	void Widget::setProperty(const std::string& _key, const std::string& _value)
	{
		...
		if (BackwardCompatibility::checkProperty(this, key, value))
		{
			...
			{
				MYGUI_LOG(Warning, "Widget property '" << key << "' have type prefix - use '" << key.substr(index + 1) << "' instead [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
	...
	{
		mWidgetChild.push_back(_widget);
		WidgetManager::GetSingleton().destroyWidget(_widget);
	}
	...
	void Widget::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
		...
		{
			MYGUI_LOG(Warning, "Widget property '" << _key << "' not found" << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
			return;
		}
