...
#include "Engine/UISystem/MyGUI_TextureUtility.h"
...
namespace MyGUI
{
	...
	{
		IResourcePtr resource = ResourceManager::GetSingleton().getByName(_name, false);
	...
	void ImageBox::frameAdvise(bool _advise)
	{
		if ( _advise )
		{
			if ( ! mFrameAdvise )
			{
				MyGUI::Gui::GetSingleton().eventFrameStart += MyGUI::newDelegate( this, &ImageBox::frameEntered );
				mFrameAdvise = true;
			}
		}
		else
