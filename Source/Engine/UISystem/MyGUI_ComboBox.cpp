...
namespace MyGUI
{
	...
	void ComboBox::notifyButtonPressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		if ( OIS::MB_Left != _id)
			return;
	...
	void ComboBox::notifyListLostFocus(Widget* _sender, Widget* _new)
	{
		...
		{
			mDropMouse = false;
			Widget* focus = InputManager::GetSingleton().getMouseFocusWidget();
	...
	void ComboBox::notifyListSelectAccept(ListBox* _widget, size_t _position)
	{
		...
		mDropMouse = false;
		InputManager::GetSingleton().setKeyFocusWidget(this);
	...
	void ComboBox::onKeyButtonPressed( OIS::KeyCode _key, Char _char)
	{
		...
		if (_key == OIS::KC_DOWN)
		{
			...
			if (!InputManager::GetSingleton().isCaptureMouse())
			{
				showList();
			}
		}
	...
	void ComboBox::notifyMouseWheel(Widget* _sender, int _rel)
	{
		if (mList->getItemCount() == 0)
			return;
		if (InputManager::GetSingleton().getKeyFocusWidget() != this)
			return;
		if (InputManager::GetSingleton().isCaptureMouse())
			return;
	...
	void ComboBox::notifyMousePressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
	...
	void ComboBox::showList()
	{
		...
		if (mShowSmooth)
		{
			...
			ControllerManager::GetSingleton().addItem(mList, controller);
		}
		else
		{
		...
		InputManager::GetSingleton().setKeyFocusWidget(mList);
	}
