...
namespace MyGUI
{
	...
	void EditBox::notifyMousePressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		if (mClientText == nullptr)
			return;
		...
		IntPoint point = InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left);
	...
	void EditBox::notifyMouseReleased(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		// сбрасываем всегда
		mMouseLeftPressed = false;
	}

	void EditBox::notifyMouseDrag(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		if (_id != OIS::MB_Left)
			return;
	...
	void EditBox::notifyMouseButtonDoubleClick(Widget* _sender)
	{
		...
		const IntPoint& lastPressed = InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left);
	...
	void EditBox::onMouseDrag(int _left, int _top, OIS::MouseButtonID _id)
	{
	...
	void EditBox::onKeySetFocus(Widget* _old)
	{
		if (!mIsPressed)
		{
			...
			if (!mModeStatic)
			{
				...
				{
					mCursorActive = true;
					Gui::GetSingleton().eventFrameStart += newDelegate(this, &EditBox::frameEntered);
					mClientText->setVisibleCursor(true);
	...
	void EditBox::onKeyButtonPressed( OIS::KeyCode _key, Char _char)
	{
		...
		{
			InputManager::GetSingleton().setKeyFocusWidget(nullptr);
		}
		...
		else if ((_key == OIS::KC_RETURN) || (_key == OIS::KC_NUMPADENTER))
		{
	...
	void EditBox::frameEntered(float _frame)
	{
		...
		if (mMouseLeftPressed)
		{
			...
			{
				IntPoint mouse = InputManager::GetSingleton().getMousePositionByLayer();
	...
	void EditBox::commandCut()
	{
		...
		{
			ClipboardManager::GetSingleton().setClipboardData(EDIT_CLIPBOARD_TYPE_TEXT, getTextSelection());
			if (!mModeReadOnly)
			{
				deleteTextSelect(true);
				// отсылаем событие о изменении
			...
			ClipboardManager::GetSingleton().clearClipboardData(EDIT_CLIPBOARD_TYPE_TEXT);
	}
	...
	void EditBox::commandPast()
	{
		...
		std::string clipboard = ClipboardManager::GetSingleton().getClipboardData(EDIT_CLIPBOARD_TYPE_TEXT);
