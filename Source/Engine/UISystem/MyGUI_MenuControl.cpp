...
namespace MyGUI
{
	...
	void MenuControl::removeItemAt(size_t _index)
	{
		...
		{
			WidgetManager::GetSingleton().destroyWidget(mItemsInfo[_index].submenu);
			mItemsInfo[_index].submenu = nullptr;
		}
		WidgetManager::GetSingleton().destroyWidget(mItemsInfo[_index].item);
	}
	...
	void MenuControl::removeAllItems()
	{
		while (!mItemsInfo.empty())
		{
			...
			{
				WidgetManager::GetSingleton().destroyWidget(mItemsInfo.back().submenu);
				mItemsInfo.back().submenu = nullptr;
			}
			WidgetManager::GetSingleton().destroyWidget(mItemsInfo.back().item);
		}
	}
	...
	void MenuControl::_setItemChildVisibleAt(size_t _index, bool _visible, bool _smooth)
	{
		...
		if (_visible)
		{
			if (mItemsInfo[_index].submenu && mItemsInfo[_index].submenu->getItemCount())
			{
				...
				MyGUI::LayerManager::GetSingleton().upLayerItem(menu);
			}
		}
		else
		{
		...
		if (mMenuDropMode)
		{
			...
			else
			{
				if (item->getItemType() == MenuItemType::Popup)
				{
					...
					item->setStateSelected(true);
					item->setItemChildVisible(true);
					InputManager::GetSingleton().setKeyFocusWidget(item);
				}
			}
		}
		else
	...
	{
		InputManager::GetSingleton().setKeyFocusWidget(_sender);
	}
	...
	void MenuControl::_wrapItemChild(MenuItem* _item, MenuControl* _widget)
	{
		...
		{
			WidgetManager::GetSingleton().destroyWidget(mItemsInfo[index].submenu);
			mItemsInfo[index].submenu = nullptr;
		}
	...
	void MenuControl::setVisible(bool _visible)
	{
		...
		if (_visible)
		{
			...
			{
				MyGUI::InputManager::GetSingleton().setKeyFocusWidget(this);
			}
		}
