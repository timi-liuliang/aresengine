...
#include "Engine/UISystem/MyGUI_ResourceLayout.h"
...
namespace MyGUI
{
	...
	Widget* ResourceLayout::createWidget(const WidgetInfo& _widgetInfo, const std::string& _prefix, Widget* _parent, bool _template)
	{
		...
		{
			MYGUI_LOG(Warning, "Root widget's layer is not specified, widget won't be visible. Specify layer or parent or attach it to another widget after load." << " [" << LayoutManager::GetSingleton().getCurrentLayout() << "]");
		}
		...
		{
			if (_parent == nullptr || style == WidgetStyle::Popup)
				coord = CoordConverter::convertFromRelative(_widgetInfo.floatCoord, RenderManager::GetSingleton().getViewSize());
			else
		...
		Widget* wid;
		if (nullptr == _parent)
			wid = Gui::GetSingleton().createWidgetT(_widgetInfo.type, _widgetInfo.skin, coord, _widgetInfo.align, widgetLayer, widgetName);
		else if (_template)
		...
		for (MapString::const_iterator iter = _widgetInfo.userStrings.begin(); iter != _widgetInfo.userStrings.end(); ++iter)
		{
			...
			if (!_template)
				LayoutManager::GetSingleton().eventAddUserString(wid, iter->first, iter->second);
		}
		...
		{
			MyGUI::ControllerItem* item = MyGUI::ControllerManager::GetSingleton().createItem(iter->type);
			if (item)
			{
				...
				MyGUI::ControllerManager::GetSingleton().addItem(wid, item);
			}
			else
			{
