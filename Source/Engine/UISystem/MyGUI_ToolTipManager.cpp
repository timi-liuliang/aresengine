...
namespace Ares
{
	...
	template <> ToolTipManager* Singleton<ToolTipManager>::m_singleton = nullptr;
}
...
namespace MyGUI
{
	...
	void ToolTipManager::initialise()
	{
		...
		Gui::GetSingleton().eventFrameStart += newDelegate(this, &ToolTipManager::notifyEventFrameStart);
	...
	{
		Widget* widget = InputManager::GetSingleton().getMouseFocusWidget();
		if (mOldFocusWidget != widget)
		{
			...
			{
				mCurrentTime = 0;
				mOldMousePoint = InputManager::GetSingleton().getMousePositionByLayer();
		...
		{
			bool capture = InputManager::GetSingleton().isCaptureMouse();
			if (capture)
			{
				if (mToolTipVisible)
				{
			...
			{
				IntPoint point = InputManager::GetSingleton().getMousePositionByLayer();
