...
namespace MyGUI
{
	...
	{
		int width = std::max<int>(1, getWidth());
		int height = std::max<int>(1, getHeight());
	...
	{
		int width = std::max<int>(1, _size.width);
		int height = std::max<int>(1, _size.height);
	...
	{
		int width = std::max<int>(1, _width);
		int height = std::max<int>(1, _height);

		destroyTexture();

		mTexture = RenderManager::GetSingleton().createTexture(mGenTexName);
	...
	{
		_width = std::max<int>(1, _width);
		_height = std::max<int>(1, _height);
	...
	void Canvas::frameAdvise( bool _advise )
	{
		if ( _advise )
		{
			if ( ! mFrameAdvise )
			{
				MyGUI::Gui::GetSingleton().eventFrameStart += MyGUI::newDelegate( this, &Canvas::frameEntered );
				mFrameAdvise = true;
			}
		}
		else
