...
#include "Engine/UISystem/MyGUI_MaskPickInfo.h"
...
namespace MyGUI
{
	...
	{
		if (!DataManager::GetSingleton().isDataExist(_file))
			return false;

		RenderManager& render = RenderManager::GetSingleton();
		ITexture* texture = render.createTexture(_file);
		texture->loadFromFile(_file);

		Texture::Modifier modifier( *texture->GetTexture(), TMA_ReadOnly);
		uint8* buffer = modifier.GetPointer<uint8>();
		if (buffer == 0)
		{
		...
		size_t pixel_size = NumFormatBytes( texture->GetTexture()->GetFormat());
