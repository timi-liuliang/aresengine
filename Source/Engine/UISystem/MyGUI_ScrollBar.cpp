...
namespace MyGUI
{
	...
	void ScrollBar::notifyMousePressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
			...
			_sender != mWidgetEnd)
		{
			if (mWidgetTrack != nullptr)
			{
				mPreActionOffset = InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left);
				const IntPoint& point = InputManager::GetSingleton().getMousePositionByLayer() - mWidgetTrack->getParent()->getAbsolutePosition();
	...
	void ScrollBar::notifyMouseReleased(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		updateTrack();
	}

	void ScrollBar::notifyMouseDrag(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		if (_id == OIS::MB_Left)
