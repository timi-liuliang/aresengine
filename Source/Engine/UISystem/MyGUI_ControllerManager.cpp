...
#include "Engine/UISystem/MyGUI_ControllerEdgeHide.h"
#include "Engine/UISystem/MyGUI_ControllerFadeAlpha.h"
#include "Engine/UISystem/MyGUI_ControllerPosition.h"
...
namespace MyGUI
{
	...
	void ControllerManager::initialise()
	{
		...
		FactoryManager::GetSingleton().registerFactory<ControllerEdgeHide>(factory_type);
		FactoryManager::GetSingleton().registerFactory<ControllerFadeAlpha>(factory_type);
		FactoryManager::GetSingleton().registerFactory<ControllerPosition>(factory_type);
	...
	void ControllerManager::shutdown()
	{
		...
		FactoryManager::GetSingleton().unregisterFactory<ControllerEdgeHide>(factory_type);
		FactoryManager::GetSingleton().unregisterFactory<ControllerFadeAlpha>(factory_type);
		FactoryManager::GetSingleton().unregisterFactory<ControllerPosition>(factory_type);
	...
	{
		IObject* object = FactoryManager::GetSingleton().createObject("Controller", _type);
	...
	void ControllerManager::addItem(Widget* _widget, ControllerItem* _item)
	{
		...
		if (mListItem.empty())
			Gui::GetSingleton().eventFrameStart += newDelegate(this, &ControllerManager::frameEntered);
