...
namespace Ares
{
	...
	template <> LayoutManager* Singleton<LayoutManager>::m_singleton = nullptr;
...
namespace MyGUI
{
	...
	void LayoutManager::initialise()
	{
		...
		ResourceManager::GetSingleton().registerLoadXmlDelegate(XML_TYPE) = newDelegate(this, &LayoutManager::_load);
		FactoryManager::GetSingleton().registerFactory<ResourceLayout>(XML_TYPE_RESOURCE);
	...
	void LayoutManager::shutdown()
	{
		...
		FactoryManager::GetSingleton().unregisterFactory<ResourceLayout>(XML_TYPE_RESOURCE);
	...
	void LayoutManager::_load(xml::ElementPtr _node, const std::string& _file, Version _version)
	{
		...
		ResourceManager::GetSingleton().addResource(resource);
	}
	...
	VectorWidgetPtr LayoutManager::loadLayout(const std::string& _file, const std::string& _prefix, Widget* _parent)
	{
		...
		if (!resource)
		{
			ResourceManager::GetSingleton().load(_file);
	...
	{
		WidgetManager::GetSingleton().destroyWidgets(_widgets);
	}
