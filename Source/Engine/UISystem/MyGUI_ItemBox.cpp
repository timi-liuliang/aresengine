...
#include "Engine/UISystem/MyGUI_WidgetTranslate.h"
...
namespace MyGUI
{
	...
	void ItemBox::findCurrentActiveItem()
	{
		...
		const IntPoint& point = InputManager::GetSingleton().getMousePositionByLayer();
	...
	void ItemBox::notifyKeyButtonPressed(Widget* _sender, OIS::KeyCode _key, Char _char)
	{
		eventNotifyItem(this, IBNotifyItemData(getIndexByWidget(_sender), IBNotifyItemData::KeyPressed, _key, _char));
	}

	void ItemBox::notifyKeyButtonReleased(Widget* _sender, OIS::KeyCode _key)
	{
	...
	void ItemBox::_resetContainer(bool _update)
	{
		...
		if ( ! _update)
		{
			WidgetManager& instance = WidgetManager::GetSingleton();
			for (VectorWidgetPtr::iterator iter = mVectorItems.begin(); iter != mVectorItems.end(); ++iter)
			{
	...
	void ItemBox::onMouseButtonPressed(int _left, int _top, OIS::MouseButtonID _id)
	{
		Base::onMouseButtonPressed(_left, _top, _id);
	}

	void ItemBox::onMouseButtonReleased(int _left, int _top, OIS::MouseButtonID _id)
	{
		Base::onMouseButtonReleased(_left, _top, _id);
	}

	void ItemBox::onMouseDrag(int _left, int _top, OIS::MouseButtonID _id)
	{
	...
	void ItemBox::updateDropItems()
	{
		if (nullptr == mItemDrag)
		{
			...
			mItemDrag = Gui::GetSingleton().createWidget<Widget>("Default", IntCoord(0, 0, coord.width, coord.height), Align::Default, mDragLayer);
			requestCreateWidgetItem(this, mItemDrag);
		}

		const IntPoint& point = InputManager::GetSingleton().getMousePosition();
	...
	void ItemBox::notifyMouseDrag(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		mouseDrag(_id);
	}

	void ItemBox::notifyMouseButtonPressed(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
		...
		if ( OIS::MB_Left == _id)
		{
			...
			mClickInWidget = InputManager::GetSingleton().getLastPressedPosition( OIS::MB_Left) - _sender->getAbsolutePosition();
	...
	void ItemBox::notifyMouseButtonReleased(Widget* _sender, int _left, int _top, OIS::MouseButtonID _id)
	{
