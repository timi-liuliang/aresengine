...
#include "Engine/UISystem/MyGUI_Bitwise.h"
...
namespace MyGUI
{
	...
	ResourceTrueTypeFont::~ResourceTrueTypeFont()
	{
		if (mTexture != nullptr)
		{
			RenderManager::GetSingleton().destroyTexture(mTexture);
			mTexture = nullptr;
		}
	}
	...
	void ResourceTrueTypeFont::initialise()
	{
		...
		bool laMode = MyGUI::RenderManager::GetSingleton().isFormatSupported(Pixel<true>::getFormat(), TextureUsage::Static | TextureUsage::Write);
	...
	void ResourceTrueTypeFont::initialiseFreeType()
	{
		...
		mTexture = RenderManager::GetSingleton().createTexture(MyGUI::utility::toString((size_t)this, "_TrueTypeFont"));

		mTexture->createManual(texWidth, texHeight, TextureUsage::Static | TextureUsage::Write, Pixel<LAMode>::getFormat());

		Ares::Texture::Modifier modifie( *mTexture->GetTexture(), Ares::TMA_WriteOnly);
		uint8* texBuffer  = modifie.GetPointer<uint8>();
	...
	FT_Face ResourceTrueTypeFont::loadFace(const FT_Library& _ftLibrary, uint8*& _fontBuffer)
	{
		...
		// Load the font file.
		IDataStream* datastream = DataManager::GetSingleton().getData(mSource);
		...
		else
		{
			...
			{
				// Some code points have been specified, so remove anything in the non-printable ASCII range as well as anything over 8 bits.
				removeCodePointRange(0, 0x1F);
				removeCodePointRange( 0x100, -1);
