...
ProjectionLight::ProjectionLight(const Transform &light2world, const Spectrum &intensity, const string &texname, float fov)
    : Light(light2world) 
{
    ...
    int width, height;
    RGBSpectrum *texels = Pbrt::ReadImage(texname, &width, &height);
    if (texels)
