...
#include <stdarg.h>				// Error Reporting Includes
...
{
	// 错误信号
	boost::signals2::signal<void(const char*)> Signal_Error;

	// process
	boost::signals2::signal<void(float)> Signal_Process;
}
...
static void processError(const char *format, va_list args, const char *message, int disposition) 
{
    ...
    else 
	{
        ...
        extern int line_num;
        int column = 0;
        int width = max(20, Pbrt::TerminalWidth() - 2);
        if (line_num != 0) 
		{
