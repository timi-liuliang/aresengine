...
namespace Pbrt
{
	...
	RGBSpectrum *ReadImage(const string &name, int *width, int *height) 
	{
		if (name.size() >= 5) 
		{
			...
			if (!strcmp(name.c_str() + suffixOffset, ".tga") || !strcmp(name.c_str() + suffixOffset, ".TGA"))
				return ReadImageTGA(name, width, height);
			if (!strcmp(name.c_str() + suffixOffset, ".pfm") || !strcmp(name.c_str() + suffixOffset, ".PFM"))
				return ReadImagePFM(name, width, height);
		}

		Error("Can't determine image file type from suffix of filename \"%s\"", name.c_str());
	...
	void WriteImage(const string &name, float *pixels, float *alpha, int xRes, int yRes, int totalXRes, int totalYRes, int xOffset, int yOffset) 
	{
		if (name.size() >= 5) {
			...
			if (!strcmp(name.c_str() + suffixOffset, ".tga") || !strcmp(name.c_str() + suffixOffset, ".TGA")) 
			{
				WriteImageTGA(name, pixels, alpha, xRes, yRes, totalXRes,totalYRes, xOffset, yOffset);
...
static void WriteImageEXR(const string &name, float *pixels, float *alpha, int xRes, int yRes, int totalXRes, int totalYRes,int xOffset, int yOffset) 
{
    ...
    for (int i = 0; i < xRes * yRes; ++i)
        hrgba[i] = Rgba(pixels[3*i], pixels[3*i+1], pixels[3*i+2], alpha ? alpha[i]: 1.f);
	...
	{
        Error("Unable to write image file \"%s\": %s", name.c_str(), e.what());
    }
...
void WriteImageTGA(const string &name, float *pixels, float *alpha, int xRes, int yRes, int totalXRes, int totalYRes, int xOffset, int yOffset)
{
    FILE*               file;
    uchar*              outBuf;
...
static RGBSpectrum *ReadImageTGA(const string &name, int *width, int *height)
{
	...
	{
        Error("ReadImageTGA: I don't know this format " "(type=%i pxsize=%i abits=%i)", header.imageType,imageSpec.pixelDepth, imageSpec.attributeBits);
...
static bool WriteImagePFM(const string &filename, const float *rgb, int width, int height) 
{
    FILE* fp;
    unsigned int nFloats;
    float scale;
