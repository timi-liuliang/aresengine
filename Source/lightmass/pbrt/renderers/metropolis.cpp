...
public:
    MLTTask( Pbrt::ProgressReporter &prog, uint32_t pfreq, uint32_t taskNum,
    ...
    if (scene->lights.size() > 0) {
        ...
        if (directLighting != NULL) {
            ...
            if (nDirectPixelSamples > 0) {
                ...
                Pbrt::ProgressReporter directProgress(nDirectTasks, "Direct Lighting");
        ...
        Pbrt::ProgressReporter progress(nTasks * largeStepRate, "Metropolis");
        vector<Task *> tasks;
...
MLTTask::MLTTask( Pbrt::ProgressReporter &prog, uint32_t pfreq, uint32_t tn,
