...
public:
	// 构造函数
    CreateRadProbeTask(int i, int nProbes[3], float t, const BBox &b, int lmax, bool id, bool ii, int nindir, Pbrt::ProgressReporter &p, Sample *sample, const vector<Point> &sp, const Scene *sc, const Renderer *ren, Spectrum *c);
...
private:
    ...
    float time;
    Pbrt::ProgressReporter &prog;							// 进度报告者
...
Spectrum CreateRadianceProbes::Transmittance(const Scene *scene, const RayDifferential &ray, const Sample *sample, RNG &rng,MemoryArena &arena) const 
{
...
void CreateRadianceProbes::Render(const Scene *scene) 
{
    ...
    vector<Task *> tasks;
    Pbrt::ProgressReporter prog(count, "Radiance Probes");
    for (int i = 0; i < count; ++i)
...
CreateRadProbeTask::CreateRadProbeTask(int pn, int d[3], float t, const BBox &b, int lm, bool id, bool ii, int nindir, Pbrt::ProgressReporter &p, Sample *samp, const vector<Point> &sp, const Scene *sc, const Renderer *ren, Spectrum *c)
...
CreateRadianceProbes *CreateRadianceProbesRenderer(const Camera *camera, SurfaceIntegrator *surf, VolumeIntegrator *vol, const ParamSet &params) 
{
