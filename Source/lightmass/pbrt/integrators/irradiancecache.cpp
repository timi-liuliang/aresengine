...
{
    IrradiancePrimeTask(const Scene *sc, const Renderer *sr, const Camera *c, Sampler *samp,Sample *s, IrradianceCacheIntegrator *ic, Pbrt::ProgressReporter &pr,int tn, int nt) 
		: progress(pr) 
	{
        scene = sc;
        renderer = sr;
    ...
    Pbrt::ProgressReporter &progress;
...
void IrradianceCacheIntegrator::Preprocess(const Scene *scene,
    ...
    const int nTasks = 64;
    Pbrt::ProgressReporter progress(nTasks, "Priming irradiance cache");
    vector<Task *> tasks;
    for (int i = 0; i < nTasks; ++i)
        tasks.push_back(new IrradiancePrimeTask(scene, renderer, camera,&sampler, sample, this, progress, i, nTasks));
    EnqueueTasks(tasks);
    WaitForAllTasks();
