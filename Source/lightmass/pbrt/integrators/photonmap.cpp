...
public:
    PhotonShootingTask(int tn, float ti, Mutex &m, PhotonIntegrator *in,
        Pbrt::ProgressReporter &prog, bool &at, int &ndp,
    ...
    Pbrt::ProgressReporter &progress;									// 进度报告
    bool &abortTasks;
    int &nDirectPaths;
...
public:
    ComputeRadianceTask( Pbrt::ProgressReporter &prog, uint32_t tn, uint32_t nt,
...
void PhotonIntegrator::Preprocess(const Scene *scene,
    ...
    Pbrt::ProgressReporter progress(nCausticPhotonsWanted+nIndirectPhotonsWanted, "Shooting photons");
    ...
    if (finalGather && radiancePhotons.size()) 
	{
        ...
        Pbrt::ProgressReporter progRadiance(numTasks, "Computing photon radiances");
