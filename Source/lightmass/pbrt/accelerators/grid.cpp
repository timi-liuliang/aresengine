...
// 构造函数
GridAccel::GridAccel(const vector<Reference<Primitive>> &p, bool refineImmediately) 
{
    ...
    for (uint32_t i = 0; i < primitives.size(); ++i)
        m_bounds = Union(m_bounds, primitives[i]->WorldBound());
    Vector delta = m_bounds.pMax - m_bounds.pMin;

    // Find _voxelsPerUnitDist_ for grid
    int maxAxis = m_bounds.GetMaximumExtent();
...
bool GridAccel::Intersect(const Ray &ray, Intersection *isect) const 
{
    ...
    float rayT;
    if ( m_bounds.Inside(ray(ray.mint)))
        rayT = ray.mint;
    else if (!m_bounds.IntersectP(ray, &rayT))
    {
