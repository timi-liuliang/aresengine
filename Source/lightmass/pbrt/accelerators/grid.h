...
private:
	...
	{
        int v = Float2Int((P[axis] - m_bounds.pMin[axis]) * invWidth[axis]);
...
private:
    vector<Reference<Primitive>> primitives;			// 场景中所有几何体
    int							 nVoxels[3];			// 在xyz轴上Voxel数量
    BBox						 m_bounds;				// 包围盒
...
GridAccel *CreateGridAccelerator(const vector<Reference<Primitive> > &prims, const ParamSet &ps);
