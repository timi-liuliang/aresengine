...
struct BVHPrimitiveInfo 
{
	...
	{
        centroid = .5f * (b.pMin + b.pMax);
    }

    int	  primitiveNumber;	// 几何体索引	
    Point centroid;			// 图心(中心点)
    BBox  bounds;			// 有向包围盒
};
...
struct BVHBuildNode 
{
    ...
    uint32_t splitAxis, 
			 firstPrimOffset,	// 叶子接点中第一成员索引
			 nPrimitives;		// 叶子接点中成员数量
};
...
// 参数1：几何体集
// 参数2：每个接点最大存储几何体数
BVHAccel::BVHAccel(const vector<Reference<Primitive> > &p, uint32_t max_pri_in_node, const string &sm) 
{
	// 每个接点最大primitive数量
    maxPrimsInNode = min(255u, max_pri_in_node);
...
BVHBuildNode *BVHAccel::recursiveBuild(MemoryArena &buildArena,
    ...
    if (nPrimitives == 1) {
        ...
        int dim = centroidBounds.GetMaximumExtent();
